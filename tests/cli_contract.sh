...
test "$("$KUJO_BIN" run redact.kujo version)" = "redact 1.1.0"
...
jq -e '.schemaVersion == "redact-cli-output/v1" and .productVersion == "1.1.0" and .command == "scan" and .detections > 0' "$TEST_TMP/scan.json" >/dev/null
...
jq -e '.schemaVersion == "redact-verifier/v1" and .productVersion == "1.1.0" and .risk_score == "low" and .remaining_payment_details == 0 and .remaining_exact_dates == 0 and .remaining_money_amounts == 0' "$TEST_TMP/verify.json" >/dev/null
